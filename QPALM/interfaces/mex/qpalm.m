...
%         objectHandle % Handle to underlying C instance
   ...
   methods(Static) 
        ...
        function out = default_settings()
            ...
            out = qpalm.qpalm_mex('default_settings');
        end
        ...
        function out = constant(constant_name)
            ...
            out = qpalm.qpalm_mex('constant', constant_name);
        end
...
%             this.objectHandle = qpalm.qpalm_mex('new', varargin{:});
        end

        %% Destructor - destroy the solver instance
        function delete(this)
            % Destroy QPALM solver class
            qpalm.qpalm_mex('delete');
        end
...
%                 A = full(A(:,:));
            ...
            %
            bmax = min(bmax, qpalm.qpalm.constant('QPALM_INFTY'));
            bmin = max(bmin, -qpalm.qpalm.constant('QPALM_INFTY'));
            ...
            [varargout{1:nargout}] = qpalm.qpalm_mex('setup',n,m,Q,q,A,bmin,bmax,theSettings);
        ...
        function warm_start(this, x, y)
            ...
            qpalm.qpalm_mex('warm_start',x,y);
        ...
        function update_bounds(this, bmin, bmax)
            ...
            qpalm.qpalm_mex('update_bounds', bmin, bmax);
        end
        ...
        function varargout = solve(this, varargin)
            ...
            [out.x, out.y, out.prim_inf_cert, out.dual_inf_cert, out.info] = qpalm.qpalm_mex('solve');
            if(nargout)
                varargout{1} = out;
            end
%             delete(this);
...
currentSettings = qpalm.qpalm_mex('default_settings');
